...
This fork is dedicated to expand this code with new functionalities that are needed for my personal PhD project (the results of which can be found in [this paper](https://iopscience.iop.org/article/10.1088/1475-7516/2024/07/021)). Here's a (ever expanding) list of changes and additions:
...
- Added a new `window` argument to apply a filter to the input field in Fourier space before the wavelet transform; this is done by initializing a window field in fourier space and passing it to [kymatio.torch.HarmonicScattering3D](https://www.kymat.io/codereference.html#module-kymatio.torch) using the `window` keyword argument; the array must be complex.
